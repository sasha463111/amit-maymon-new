...
- **Enum expansion**: `approval_type` was extended with `CASE_CLOSURE` after 001 — the enum value still exists, but the app stopped creating/checking it in Session 6 (`CLOSE_CASE` no longer requires it, see `bodyshop-workflow.md`). Same story for `WHEELS_CHECK` as of 2026-08-31/migration 044 — the enum value still exists (old rows too), but the app no longer creates or gates on it, only sends a plain notification. A fresh setup must still include all migrations or the enum will be incomplete.
